...
| Skills Registry.app | Swift (SwiftUI) | macOS app bundle, arm64 (built by `.github/workflows/release-macapp.yml`) | Native desktop GUI for the same registry: GitHub App Device-Flow login, browse with rich markdown + fuzzy search, publish/remove, bulk-import local skills, 1-click CLI install + copyable MCP JSON. Shares `registry.toml` and the slug/frontmatter/fuzzy contract with the CLI. Its CLI PATH warning probes the login shell, since Finder-launched apps do not inherit shell startup PATH changes. |
